          ...
          {session && (
            <Menu>
              <MenuButton  transition="all 0.2s" color={'black'}>
                <HStack>
                  <Text fontWeight={'semibold'}>Olá, {user?.name} </Text>
                  <IoMdArrowDropdown />
                </HStack>
              </MenuButton>
              <MenuList color='black'>
                <MenuItem onClick={() => signOut()} >Sair</MenuItem>
                <MenuItem><Link href={'/carrinho'}>Meus produtos</Link></MenuItem>
              </MenuList>
            </Menu>
          )}
          <Text color={'black'} transition={'0.5s'} _hover={{color:'#FF5C01'}} cursor={'pointer'}>
